...
class ClientTestBase:
    ...
    def test_request(self):
        ...
        for meth in VALID_API_METHODS:
            ...
            body, code = self.make_request(meth, abs_url, headers, data)
    ...
    def test_custom_timeout(self):
        custom_timeout = 3
        client = self.make_client(verify_ssl_certs=True, timeout_in_seconds=custom_timeout)
        method = "post"
        abs_url = self.valid_url
        data = ""
        headers = {"my-header": "header val"}
        ...
        self.check_call_with_timeout(self.request_mock, method, abs_url, data, headers, custom_timeout)

    def test_exception(self):
        self.mock_error(self.request_mock)
        self.assertRaises(shippo.error.APIConnectionError, self.make_request, "get", self.valid_url, {}, None)
...
class RequestsClientTests(ShippoUnitTestCase, ClientTestBase):
    ...
    def check_call_with_timeout(self, mock, meth, url, post_data, headers, timeout):
        mock.request.assert_called_with(meth, url, headers=headers, data=post_data, timeout=timeout)

    def check_call(self, mock, meth, url, post_data, headers):
        self.check_call_with_timeout(mock, meth, url, post_data, headers, shippo.http_client.RequestsClient.DEFAULT_TIMEOUT)
...
class UrlFetchClientTests(ShippoUnitTestCase, ClientTestBase):
    ...
    def check_call_with_timeout(self, mock, meth, url, post_data, headers, timeout):
        mock.fetch.assert_called_with(url=url, method=meth, headers=headers, validate_certificate=True, deadline=timeout, payload=post_data)

    def check_call(self, mock, meth, url, post_data, headers):
        self.check_call_with_timeout(mock, meth, url, post_data, headers, shippo.http_client.UrlFetchClient.DEFAULT_TIMEOUT)
